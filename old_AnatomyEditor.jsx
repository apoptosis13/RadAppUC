...
const AnatomyEditor = ({ series, onUpdate }) => {
    ...
    const handleClearAnnotations = (category = 'all') => {
        const confirmMsg = category === 'all'
            ? "┬┐Est├ís seguro de eliminar TODAS las anotaciones de esta serie?"
            : `┬┐Est├ís seguro de eliminar todas las anotaciones de la categor├¡a "${category}"?`;
    ...
    const handleDeleteAnnotation = (id) => {
        ...
        if (structureIndex >= 0) {
            ...
            // "Me gustar├¡a que al agregar una estructura en una serie, esta quede disponible en toda la serie"
    ...
    const handleDeleteSeriesStructure = (id) => {
        if (!window.confirm(t('┬┐Est├ís seguro de que quieres eliminar esta estructura de TODA la serie? Esta acci├│n no se puede deshacer.'))) return;
    ...
    const addSeriesStructure = (type, data) => {
        ...
        const newStructure = {
            id: newId,
            label: type === 'polygon' ? 'M├║sculo/Regi├│n' : 'Estructura',
    ...
    const handleReverseSeries = () => {
        if (!window.confirm('┬┐Invertir el orden de toda la serie? Las anotaciones se ajustar├ín autom├íticamente.')) return;
    ...
    if (!currentImage) return <div className="text-center p-10 text-gray-500">No hay im├ígenes en esta serie</div>;
                    ...
                    <div className="flex space-x-2">
                        ...
                        <div className="flex items-center space-x-4">
                            <div className="flex items-center space-x-2">
                                <span className="text-sm text-gray-400">Categor├¡a:</span>
                                <select
                            ...
                            <div className="flex items-center space-x-2">
                                <span className="text-sm text-gray-400">Propagaci├│n:</span>
                                <select
                                    value={propagationRange}
                                    onChange={(e) => setPropagationRange(e.target.value)}
                                    className="text-sm bg-gray-700 border-gray-600 text-white rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                    title="En cu├íntas im├ígenes se replicar├í la estructura creada"
                                >
                        ...
                        {showManagement && (
                            <div className="mt-3 p-3 bg-gray-900 rounded-md border border-gray-600 space-y-3 animate-in slide-in-from-top-2 duration-200">
                                ...
                                <select
                                    ...
                                    defaultValue=""
                                >
                                    <option value="" disabled>Borrar por categor├¡a...</option>
                                    {ANATOMY_CATEGORIES.map(cat => (
                                        <option key={cat.id} value={cat.id}>{cat.label}</option>
                                    ))}
                                </select>
                                ...
                                <div>
                                    <div className="flex justify-between items-center mb-1">
                                        ...
                                        <button
                                            onClick={async () => {
                                                ...
                                                if (ann.description) {
                                                    ...
                                                    try {
                                                        ...
                                                        alert("Error al traducir. Verifica tu conexi├│n.");
                                                    } finally {
                                            ...
                                            title="Traducir descripci├│n con Google Cloud AI"
                                        >
                                            {isTranslating ? (
                                ...
                                <div className="pt-4 border-t border-gray-700 mt-4 space-y-2">
                                    ...
                                    {seriesStructures.some(s => s.id === selectedAnnotationId) && (
                                        <button
                                            ...
                                            title="Eliminar de todas las im├ígenes de la serie"
                                        >
                                            <Trash2 className="w-4 h-4 mr-2" />
